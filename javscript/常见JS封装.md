...
function parents(ele,n){
	while(ele && n){
    ele = ele.parentElement? ele.parentElement : eleParentNode;
    n--;
  }
  return ele;
}
...
function retSibling(e,n){
  while(e&& n){
    if(n>0){
      if(e.nextElementSibiling){
        e = e.nextElementSibiling;
      }else{
      ...
      if(e.previousElementSibling){
        ...
        for(e = e.previousElementSibling; e && e.noteType !== 1;e = e.previousElementSibling);
      }
      n++
    }
...
function hasChildren(e){
	var children = e.childNodes, len = children.lenth;
	for(var i = 0 ; i < len ; i++){
		if(children[i].nodeType === 1){
			reutnr true;
		}
...
function getScrollOffset(){
  if(window,pageXoffset){
    ...
    return {
      ...
      y: document.body.scrollRight = docuemnt.documentElement.scrollRight
    }
  }
}
```
...
function debounce(handle, delay) {
    var timer = null;// 定义timer在外面 （闭包） 多个函数公用一个外部变量
    return function () {
        var _self = this,
            _args = arguments;
        clearTimeout(timer);
